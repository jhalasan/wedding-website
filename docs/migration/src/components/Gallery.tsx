...
export default function Gallery() {
  ...
  return (
    <section id="gallery" style={{ background: c.sand, borderTop: `1px solid ${c.line}`, borderBottom: `1px solid ${c.line}`, padding: sectionPad }}>
          ...
          <div style={{ position: "relative" }}>
            ...
            >
              {GALLERY.map((src, i) => (
                ...
                >
                  <img src={src} alt="Julius and Revia" loading="lazy" style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                </button>
              ))}
            </div>
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: "1rem", marginTop: "1.1rem" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: ".65rem" }}>
              ...
              <div style={{ display: "flex", gap: ".45rem", alignItems: "center" }}>
                {GALLERY.map((src, i) => (
                  <button key={src} type="button" aria-label="Go to photo" onClick={() => scrollToIndex(i)} style={{ width: 6, height: 6, borderRadius: "50%", background: i === active ? c.green : c.line, border: "none", padding: 0, cursor: "pointer" }} />
                ))}
              </div>
            </div>
      ...
      {lightbox !== null && (
        <div onClick={() => setLightbox(null)} style={{ position: "fixed", inset: 0, zIndex: 60, background: "rgba(28,40,27,.94)", display: "flex", alignItems: "center", justifyContent: "center", padding: "clamp(1rem,4vw,2.5rem)" }}>
          <button type="button" aria-label="Close" onClick={() => setLightbox(null)} style={{ position: "absolute", top: "1.2rem", right: "1.4rem", width: 40, height: 40, border: `1px solid ${c.white}`, borderRadius: "50%", background: "none", color: c.white, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", fontSize: "1.3rem", lineHeight: 1 }}>×</button>
          <img src={GALLERY[lightbox]} alt="Julius and Revia" onClick={(e) => e.stopPropagation()} style={{ maxWidth: "min(94vw,900px)", maxHeight: "88svh", objectFit: "contain" }} />
        </div>
      )}
    </section>
  );
